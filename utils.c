...
bool setup_prepared_stmt(MYSQL_STMT **stmt, char *statement, MYSQL *conn)
{
	...
	*stmt = mysql_stmt_init(conn); /*Allocates and initializes a MYSQL_STMT handler. Returns a pointer to the handler, or
									NULL if the handler could not be allocated.*/
	if (*stmt == NULL)
	{
	...
	if (mysql_stmt_prepare (*stmt, statement, strlen(statement)) != 0) {  //to send a statement to the server to be prepared and associated with the statement handler.
